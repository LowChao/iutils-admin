 ...
 * @param timestamp
 */
export const formatDateUtc = function(timestamp, fmt) {
	let d = new Date(timestamp)
	let timezone = new Date().getTimezoneOffset() / 60
	let targetDate = new Date(d.getTime() - timezone * 60 * 60 * 1000)
	if(fmt){
		return formatDate(targetDate.getTime(), fmt)
	}
 ...
 * @param path
 */
export const getActiveMenu = function(menus, path){
	let flag = false, activeMenu = null, activeName = null
	for(let i=0;i<menus.length;i++){
		let oneMenu = menus[i]
		if(oneMenu.value != "#"){
			if(path && path!="/" && path==oneMenu.value){
				activeMenu = oneMenu.value
				activeName = oneMenu.text
				flag = true
				break
			}
			if(!path){
		...
		if(!flag && oneMenu.children){
			for(let j=0;j<oneMenu.children.length;j++){
				let twoMenu = oneMenu.children[j]
				if(twoMenu.value != "#"){
					if(path && path!="/" && path==twoMenu.value){
						activeMenu = twoMenu.value
						activeName = twoMenu.text
						flag = true
						break
					}
					if(!path){
				...
				if(!flag && twoMenu.children){
					for(let k=0;k<twoMenu.children.length;k++){
						let threeMenu = twoMenu.children[k]
						if(threeMenu.value != "#"){
							if(path && path!="/" && path==threeMenu.value){
								activeMenu = threeMenu.value
								activeName = threeMenu.text
								flag = true
								break
							}
							if(!path){
