...
//模块接口
var cache_mid_url = '/module/getModule'
...
export default class Request {
	async asyncHttp(param) {
		...
		if(cache_dict_url == url){
			//获取字典缓存
			let res_dict = sessionStorage.getItem("DICT-"+data["code"])
			if(res_dict){
		...
		if(res.data.code == 10000){
			...
			if(res.data.code == 20000){
				if(cache_dict_url == url && res.data.data){
					//缓存字典
					sessionStorage.setItem("DICT-"+data["code"], JSON.stringify(res.data))
				}else if(cache_mid_url == url && res.data.data){
					//缓存模块
